    ...
    if (!propertyId) {
        main.innerHTML = `
            <div class="property-not-found">
                <h2>${window.languageManager.translate('property_not_found')}</h2>
                <p>${window.languageManager.translate('property_not_found_desc')}</p>
                <a href="index.html#properties" class="btn btn-primary">${window.languageManager.translate('property_view_all')}</a>
            </div>
        `;
        return;
    }
    ...
    main.innerHTML = `
        <div class="property-detail">
            ...
            <div class="property-content">
                <div class="container">
                    <a href="index.html#properties" class="back-to-properties">${window.languageManager.translate('property_back')}</a>

                    <div class="property-header">
                        ${property.featured ? `<span class="property-badge-featured">${window.languageManager.translate('property_featured')}</span>` : ''}
                        <h1 class="property-title">${property.title}</h1>
                        <p class="property-location">📍 ${property.location}</p>
                        <p class="property-price-large">${window.languageManager.translate('aed')} ${formatPrice(property.price)}</p>

                        <!-- Social Share Section -->
                        <div class="share-section">
                            <h4 class="share-title">${window.languageManager.translate('property_share')}</h4>
...
function copyToClipboard(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
            showNotification(window.languageManager.translate('share_copied'), 'success');
        }).catch(err => {
...
function fallbackCopyToClipboard(text) {
    ...
    try {
        ...
        showNotification(window.languageManager.translate('share_failed'), 'error');
    }
...
document.addEventListener('languageChanged', (e) => {
    // Re-render property details with new language
    renderPropertyDetail();
});
